...
namespace API.Controllers
{
    public class AdminController : BaseApiController
    {
        ...
        public async Task<ActionResult> EditRole(string username, [FromQuery] string roles)
        {
            ...
            var result = await userManager.AddToRolesAsync(user, selectedRoles.Except(userRole));

            if (!result.Succeeded) return BadRequest("Failed to add roles");

            result = await userManager.RemoveFromRolesAsync(user, userRole.Except(selectedRoles));
